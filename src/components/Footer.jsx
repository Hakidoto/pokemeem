...
const Footer = ({pokemon, processHealth, handleClickLog, conVida, pokemonEnemy}) => {
  return (
    <div className="playgroundFooter">
       {pokemon.map(({name}) => (
        <p key={0} className="footerP">Que deberia hacer {name.charAt(0).toUpperCase() + name.slice(1)}? </p>
       ))}  
        <ButtonSpace conVida={conVida} handleClickLog={handleClickLog} pokemon={pokemon} processHealth={processHealth} pokemonEnemy={pokemonEnemy} />
      </div>
  )
}
